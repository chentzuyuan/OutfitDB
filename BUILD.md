# Building ClosetMind as a desktop app

ClosetMind ships as a downloadable `.app` (macOS) / `.exe` (Windows) /
...
```bash
cd closetmind
.venv/bin/python -m tools.build_app    # produces dist/ClosetMind.app  (~172 MB)
.venv/bin/python -m tools.make_dmg     # produces dist/ClosetMind-0.1.0.dmg (~83 MB compressed)
```

**For development testing** (just want to launch the .app yourself):
just run `tools.build_app` and double-click `dist/ClosetMind.app`. The
...
1. Download `ClosetMind-0.1.0.dmg`.
2. Double-click the DMG → window opens showing **ClosetMind.app** next
   to an **Applications** folder shortcut.
3. Drag ClosetMind.app onto Applications. Installed.
4. Launch from Spotlight (Cmd+Space → "ClosetMind"), Launchpad, or
   Finder → Applications.
...
3. Runs `pyinstaller closetmind.spec --noconfirm`.

The actual bundling logic is in `closetmind.spec`. It pre-handles three
gotchas that always bite FastAPI-stack apps the first time you freeze
them:

| Gotcha | How `closetmind.spec` handles it |
|---|---|
...
`app/version.py` holds `APP_VERSION = "0.1.0"`. Bump it when releasing
a new build. The footer on every page shows this version.

## Auto-update mechanism

The app checks `https://raw.githubusercontent.com/buttegg/closetmind-releases/main/latest.json`
...
"New version available →" link pointing at the manifest's `url`.

Manifest format:
```json
{
  "version": "0.2.0",
  "url": "https://github.com/buttegg/closetmind-releases/releases/download/v0.2.0/ClosetMind.dmg",
...
3. Distribute `dist/ClosetMind.app` (zip it for transport — macOS extended
...
`CLOSETMIND_UPDATE_FEED` to a different URL before launching.

## macOS Gatekeeper

Unsigned `.app` bundles are blocked by macOS Gatekeeper on download.
End users see a "ClosetMind cannot be opened because the developer
...
|---|---|
| `ModuleNotFoundError: No module named 'X'` at runtime | Add `X` to `hiddenimports` in `closetmind.spec` |
...
| `dist/ClosetMind.app` won't open (Gatekeeper) | Right-click → Open the first time |
...
- [ ] `python -m tools.build_app` — produces `dist/ClosetMind.app`
- [ ] Test the bundle launches and `/setup` works on a fresh data dir
- [ ] Test on a clean profile (`mv ~/.closetmind ~/.closetmind.bak`, then re-launch)
- [ ] `python -m tools.make_dmg` — produces `dist/ClosetMind-X.Y.Z.dmg`
